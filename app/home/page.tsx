    ...
    if (token) {
        useEffect(() => {
            if(search.length == 0){
                const getCar = async () => {
                    try {
                        const response = await axios.get('https://bemobi.vercel.app/car', {
                            headers: {
                ...
                getCar()
                console.log("ini ga search : ", carData)
            } else {
                const getCar = async () => {
                    setIsLoading(true);
                    try {
                        const response = await axios.get(`https://bemobi.vercel.app/car/${search}`, {
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },
                        })
                ...
                getCar()
                console.log("ini search : ", carData)
            }
        }, [search, token])
        return (
            <>
                ...
                <div className="mt-[70px]">
                    <div>
                        <img src="/home/bghome.png" className="w-[1600px]"></img>
                        <div className="flex justify-center mt-[-30px]">
                            <div className="bg-[#00ADB5] p-1 rounded-lg pr-[10px]">
                                <input type="text" placeholder="Cari mobil yang anda inginkan" className="w-[1200px] h-[60px] border border-gray-300 rounded-lg px-4" onChange={(e) => setPendingSearch(e.target.value)}/>
                                <button className="ml-[10px] bg-[#00393C] font-bold text-[#00ADB5] px-4 py-2 rounded-md w-[90px] h-[50px]" onClick={() => setSearch(pendingSearch)}>{isLoading ? "Loading.." : "Cari"}</button>
                            </div>
                        </div>
                        ...
                        <section id="rent">
                            <div className="grid grid-cols-3 mt-[60px]">
                                {
                                    carData && carData.length && carData.map((car: any) => {
                                        return (
                                            <>
                                                <ProductCard key={car.carID} image={car.image} name={car.name} price={car.price} model={car.model} am={car.am} color={car.color} />
                                            </>
                                        )
                                    })
                                }
                        ...
                        <footer className="bg-gray-300 py-4">
                            <div className="container mx-auto text-center">
                                <p className="text-gray-300">© 2023 MobiGo. All rights reserved.</p>
                            </div>
                        </footer>
                    </div>
                </div>
